...
export class RewardController {
   ...
   */
  private getTierCashbackRate(points: number): number {
    if (points >= 10000) return 0.01;  // Diamond  1%
    if (points >= 5000)  return 0.01;  // Platinum 1%
    if (points >= 2000)  return 0.007; // Gold     0.7%
    if (points >= 500)   return 0.005; // Silver   0.5%
   ...
   */
  async awardCashback(orderId: string): Promise<void> {
    const order = await Order.findById(orderId).select('user total orderNumber paymentStatus cashbackAwarded');
    if (!order || order.paymentStatus !== 'completed') return;
    if ((order as any).cashbackAwarded) return; // idempotency guard

    const user = await User.findById(order.user).select('points');
    if (!user) return;

    const rate = this.getTierCashbackRate(user.points || 0);
    if (rate === 0) return; // Bronze — no cashback

    const cashbackAmount = Math.round(order.total * rate * 100) / 100;
    if (cashbackAmount < 1) return; // too small to bother

    const tierName = this.getTierName(user.points || 0);

    // Mark the order so we never double-credit
    await Order.findByIdAndUpdate(orderId, { cashbackAwarded: true });
    ...
    logger.info(`✅ Cashback ₦${cashbackAmount} (${Math.round(rate * 100)}% ${tierName}) awarded to user ${order.user} for order ${order.orderNumber}`);

    try {
      await notificationService.pointsEarned(
        order.user.toString(),
        0,
        `You earned ₦${cashbackAmount.toLocaleString()} cashback as a ${tierName} member! It has been added to your wallet.`
      );
    } catch (err) {
      logger.error('Error sending cashback notification:', err);
    }
  }
}
