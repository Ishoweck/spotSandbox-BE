...
export class RewardController {
  ...
  async checkBadges(userId: string): Promise<void> {
    ...
    if (user.role === UserRole.VENDOR) {
      const vendorProfile = await VendorProfile.findOne({ user: userId }).select('verificationStatus averageRating totalReviews');
  ...
  private async checkVendorBadges(
    userId: string,
    user: any,
    vendorProfile: any,
  ): Promise<void> {
    ...
    if (salesCount >= 1   && !badges.includes('vendor-first-sale'))    await this.awardBadge(userId, 'vendor-first-sale');
    if (salesCount >= 10  && !badges.includes('vendor-ten-sales'))     await this.awardBadge(userId, 'vendor-ten-sales');
    if (salesCount >= 50  && !badges.includes('vendor-fifty-sales'))   await this.awardBadge(userId, 'vendor-fifty-sales');
    if (salesCount >= 100 && !badges.includes('vendor-century-seller')) await this.awardBadge(userId, 'vendor-century-seller');
    ...
    if (revenue >= 50000   && !badges.includes('vendor-revenue-50k'))   await this.awardBadge(userId, 'vendor-revenue-50k');
    if (revenue >= 500000  && !badges.includes('vendor-revenue-500k'))  await this.awardBadge(userId, 'vendor-revenue-500k');
    if (revenue >= 1000000 && !badges.includes('vendor-millionaire'))   await this.awardBadge(userId, 'vendor-millionaire');
    ...
    if (productCount >= 1  && !badges.includes('vendor-first-listing')) await this.awardBadge(userId, 'vendor-first-listing');
    if (productCount >= 10 && !badges.includes('vendor-ten-products'))  await this.awardBadge(userId, 'vendor-ten-products');
    if (productCount >= 50 && !badges.includes('vendor-prolific'))      await this.awardBadge(userId, 'vendor-prolific');

    // ── Reviews received ─────────────────────────────────────────
    if (!badges.includes('vendor-five-star') || !badges.includes('vendor-highly-rated')) {
      const vendorProductIds = await Product.distinct('_id', { vendor: vid });

      if (!badges.includes('vendor-five-star')) {
        const fiveStarExists = await Review.exists({
          product: { $in: vendorProductIds },
          rating: 5,
        });
        if (fiveStarExists) await this.awardBadge(userId, 'vendor-five-star');
      }

      if (
        !badges.includes('vendor-highly-rated') &&
    ...
    if (!badges.includes('vendor-early')) {
      ...
      if ((user as any).createdAt <= earlyVendorCutoff) {
   ...
   */
  async awardCustomerReferralPoints(buyerUserId: string, orderId: string): Promise<void> {
    const REFERRER_BONUS = 200;

    const buyer = await User.findById(buyerUserId).select('referredBy');
    if (!buyer?.referredBy) return;

    // Only on first completed order
    const completedCount = await Order.countDocuments({ user: buyerUserId, status: 'delivered' });
    ...
    await this.awardPoints(
      buyer.referredBy.toString(),
      REFERRER_BONUS,
      'referral',
      'Your referral made their first purchase!',
      { referredUserId: buyerUserId, orderId }
    );
  }
   ...
   */
  private getTierConversionRate(points: number): number {
    if (points >= 10000) return 1.00; // Diamond  ₦1.00/pt
    if (points >= 5000)  return 1.00; // Platinum ₦1.00/pt
    if (points >= 2000)  return 0.70; // Gold     ₦0.70/pt
    if (points >= 500)   return 0.50; // Silver   ₦0.50/pt
