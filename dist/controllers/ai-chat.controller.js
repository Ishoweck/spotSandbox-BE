...
class AIChatController {
    async adminSuggest(req, res) {
        try {
            const { messages, userRole, userName } = req.body;
            if (!Array.isArray(messages) || messages.length === 0) {
                throw new error_1.AppError('Messages are required', 400);
            }
            ...
            try {
                ...
                suggestions = arr
                    .filter((s) => s && typeof s.title === 'string' && typeof s.reply === 'string')
                    .slice(0, 5);
            }
            catch {
                suggestions = [];
            ...
            throw new error_1.AppError('Failed to generate suggestions', 500);
        }
    }
    async chat(req, res) {
        try {
            ...
            if (message.length > 2000) {
                throw new error_1.AppError('Message is too long (max 2000 characters)', 400);
            }
