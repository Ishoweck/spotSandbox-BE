...
class VendorController {
     ...
     */
    async submitBusinessSurvey(req, res) {
        const vendorProfile = await VendorProfile_1.default.findOne({ user: req.user?.id });
        if (!vendorProfile) {
            throw new error_1.AppError('Vendor profile not found', 404);
        }
     ...
     */
    async getVendorLeaderboard(req, res) {
        const period = req.query.period || 'month'; // 'month' | 'alltime'
        ...
        const leaderboard = results.map((v, index) => ({
            ...
            verified: v.verificationStatus === types_1.VendorVerificationStatus.VERIFIED,
        ...
        if (req.user?.id) {
            ...
            if (myPosition !== -1) {
                ...
                const currentVendorId = new mongoose_1.Types.ObjectId(req.user.id);
                const [myRevResult] = await Order_1.default.aggregate([
                ...
                if (myRevResult) {
                    const [rankResult] = await Order_1.default.aggregate([
                        { $match: { paymentStatus: 'completed', ...dateFilter } },
                        { $unwind: '$items' },
                        {
                            $group: {
