...
const DeferredLink_1 = require("../models/DeferredLink");
...
async function store(req, res) {
    ...
    await DeferredLink_1.DeferredLink.create({ affiliateCode, ipAddress, platform, timezone, language });
...
async function resolve(req, res) {
    ...
    const candidates = await DeferredLink_1.DeferredLink.find({ ipAddress, used: false })
        .sort({ createdAt: -1 })
        .limit(5)
        .lean();
    ...
    for (const c of candidates) {
        let score = 0;
        if (platform !== 'unknown' && c.platform === platform)
            score += 3;
    ...
    await DeferredLink_1.DeferredLink.findByIdAndUpdate(best._id, { used: true });
    res.json({ success: true, affiliateCode: best.affiliateCode });
}
//# sourceMappingURL=deferred-link.controller.js.map